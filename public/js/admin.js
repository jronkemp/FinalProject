...
function showAllPosts() {
    const url = baseUrl_API + "/posts?limit=50";
    fetch(url, {
        method: 'GET',
        ...
        .catch(err => showPosts("Errors", err)) //display errors
}
...
function displayAllPosts(posts, subheading=null) {
    console.log("display all posts for the editing purpose")
    ...
    //the row of element for adding a new message
        ...
        // add new post button
        _html += `<div class='content-row post-add-button-row'><div class='post-add-button' onclick='showAddRow()'>+ ADD A NEW POST</div></div>`;

    //Finally, update the page
    subheading = (subheading == null) ? 'All posts' : subheading;
    updateMain('Posts', subheading, _html);
}
...
function searchPosts() {
    ...
    //console.log(term);
    const url = baseUrl_API + "/posts?q=" + term;
    let subheading = '';
    //console.log(url);
    if (term == '') {
        subheading = "All Posts";
    } else if (isNaN(term)) {
        subheading = "Post Containing '" + term + "'"
    } else {
        subheading = "Posts whose ID is having" + term;
    }
//send the request
    fetch(url, {
        method: 'GET',
...
function removePost(id) {
    ...
    fetch(url, {
        ...
        .then(checkFetch)
        .then(() => showAllPosts())
        .catch(error => showPosts("Errors", error))
}
