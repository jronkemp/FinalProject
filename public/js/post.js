...
function showPosts () {
    ...
    function showPosts(offset = 0) {
        ...
        const url = baseUrl_API + '/messages?limit=' + limit + "&offset=" + offset +
            "&sort=" + sort;
...
//Callback function: display all posts; The parameter is a promise returned by axios request.
    function displayPosts(response) {
        ...
        updateMain('Posts', 'All Posts', _html);
    }
    ...
    */
    function showComments(number) {
        let url = baseUrl_API + '/posts/' + number + '/comments';
        axios({
            method: 'get',
            url: url,
            cache: true,
...
// Parameters: course number, a promise
    ...
    function handleAxiosError(error) {
        ...
        if (error.response) {
            ...
            error
            errPost = JSON.stringify(error.message, null, 4);
        }
        showPosts('Error', errPost);
    }
}
...
onclick='showPosts(${pages.first})'> << </a>
                <a href='#course' title="previous page" 
onclick='showPosts(${pages.prev})'> < </a>
                <a href='#course' title="next page" 
onclick='showPosts(${pages.next})'> > </a>
                <a href='#course' title="last page" 
onclick='showPosts(${pages.last})'> >> </a>`;
    return _html;
}
...
function limitPosts(response) {
    ...
    let _html = `&nbsp;&nbsp;&nbsp;&nbsp; Items per page:<select id='post-limit-
select' onChange='showPosts()'>`;
